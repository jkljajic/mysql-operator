...
import (
	...
	agentMetrics "github.com/jkljajic/mysql-operator/pkg/util/metrics"
...
var scrapers = map[collector.Scraper]bool{
	...
	collector.ScrapeAutoIncrementColumns{}:        true,
	collector.ScrapeBinlogSize{}:                  true,
	...
	collector.ScrapePerfFileEvents{}:              true,
...
func newHandler(metrics collector.Metrics, scrapers []collector.Scraper, logger log.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		agentMetrics.CreatePrometheysRegistry()
		agentMetrics.MustRegister(collector.New(ctx, dsn, metrics, filteredScrapers))
		...
		gatherers := prometheus.Gatherers{
			...
			agentMetrics.PrometheusRegistry,
		}
...
func mysql_collector() {
	...
	kingpin.Flag("verbose", "Verbose mode").Short('v').Int32()
...
`)
	...
	if len(dsn) == 0 {
		...
		dsn = fmt.Sprintf("root:%s@(localhost:3306)/", rootPass)
	}
