...
func NewForCluster(cluster *v1alpha1.Cluster) *corev1.Service {
	...
	svc := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			...
			Name:      cluster.GetServiceHeadlessName(),
...
// NewForClusterPrimary will return a new headless Kubernetes service for a MySQL cluster
func NewForClusterPrimary(cluster *v1alpha1.Cluster) *corev1.Service {
	mysqlPort := corev1.ServicePort{Port: 3306}

	svc := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Labels:    map[string]string{constants.ClusterLabel: cluster.Name},
			Name:      cluster.GetServiceName(),
			Namespace: cluster.Namespace,
			OwnerReferences: []metav1.OwnerReference{
				*metav1.NewControllerRef(cluster, schema.GroupVersionKind{
					Group:   v1alpha1.SchemeGroupVersion.Group,
	...
	if cluster.RequiresServicePrimary() {
		svc.Spec.Selector[constants.LabelClusterRole] = "primary"
	}
	return svc
}
