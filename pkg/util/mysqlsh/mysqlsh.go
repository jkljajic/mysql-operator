...
type Interface interface {
	...
	ForceQuorumUsingPartitionOf(ctx context.Context, uri string) error
}
...
//TODO: checkINstance do not worksin check only if is error default == already belongs to the cluster: 'default'.
...
func (r *runner) ForceQuorumUsingPartitionOf(ctx context.Context, uri string) error {
	python := fmt.Sprintf("dba.get_cluster('%s').force_quorum_using_partition_of('%s')", innodb.DefaultClusterName, uri)
	_, err := r.run(ctx, python)
	return err
}

func (r *runner) RescanCluster(ctx context.Context) error {
	python := fmt.Sprintf("dba.get_cluster('%s').rescan({'removeInstances':'auto'})", innodb.DefaultClusterName)
	_, err := r.run(ctx, python)
	return err
}

func (r *runner) SetPrimaryInstance(ctx context.Context, uri string) error {
	python := fmt.Sprintf("dba.get_cluster('%s').set_primary_instance('%s')", innodb.DefaultClusterName, uri)
	_, err := r.run(ctx, python)
	return err
}
...
func (r *runner) stripPasswordWarning(in []byte) []byte {
	warning := []byte("[33mWARNING: [0mUsing a password on the command line interface can be insecure.\n")
...
func (r *runner) run(ctx context.Context, python string) ([]byte, error) {
	...
	args := []string{"--no-wizard", "--auth-method=caching_sha2_password", "--uri", r.uri, "--py", "-e", python} //"--quiet-start=2",
	...
	klog.V(6).Infof("Running command: mysqlsh %v", args)
	//TODO: ignore error : unable to resolve the address as either an IPv4 or IPv6 host
	err := cmd.Run()
	klog.V(6).Infof("    stdout: %s\n    stderr: %s\n    err: %s", stdout, r.stripPasswordWarning(stderr.Bytes()), err)

	if err != nil {
		underlying := NewErrorFromStderr(stderr.String())
		if underlying != nil {
			return nil, underlying //errors.WithStack(
		}
	}
...
func (r *runner) RebootClusterFromCompleteOutage(ctx context.Context) error {
	...
	python := fmt.Sprintf("session.run_sql('SET GLOBAL group_replication_group_seeds = \"\"')")
	_, err := r.run(ctx, python)
	if err != nil {
		return err
	}
